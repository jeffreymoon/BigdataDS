...
def encoding(data):
    ...
    # ========== EDIT HERE ==========
    # zero_arr = np.zeros((len(data)+1, 26))
    # one_arr = np.ones((len(data)+1, 1))
    # arr = np.concatenate((zero_arr, one_arr), axis=1)
    # pack_bit = np.packbits(arr, axis=-1)
    # print(pack_bit)

    encoded = np.array((np.eye(1, 27, (alph.index(c) for c in data))))
    print(encoded)
    # print(ord('a') == 97)
